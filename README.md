...
- 采用版本化规则的天乙贵人、禄神、驿马、华盖、将星，显示查法依据且不输出吉凶；咸池因古籍另有纳音条件暂不简化输出；
- 日期控制器：今天、上/下一年、上/下一月、上/下一天、当前时辰、自定义日期；
- URL 保存查看层级与日期，本地随机编号恢复出生资料，不在 URL 暴露完整信息；
- 复制、PNG / 长图、PDF、浏览器打印和匿名导出；
- 纯白极简响应式界面，以五行专属文字色区分干支属性；桌面端使用轻量液态玻璃与淡入动效，移动端自动关闭模糊，并支持减少动态、高对比度和打印样式；
...
仓库中的 [`mobile/`](mobile/) 是同一排盘算法驱动、名为“元序”的 Expo / React Native App。目前已经实现本地多命盘档案库、原盘、细盘、流通，以及大运、流年、流月、流日、流时。iOS 26/27 独立构建使用系统 Liquid Glass，Expo Go 与其他平台采用兼容性降级。启动、未签名 IPA 和签名边界见 [`mobile/README.md`](mobile/README.md)。
...
本项目没有复制 GitHub UI 模板：工程骨架和纯白极简界面均为原创，因此无模板来源或模板许可证附加要求，也没有复制其他排盘网站或 Apple 的商标、图标、字体文件和受保护素材。
...
- 神煞当前开放 v3 固定目录中的 40 项可复核表诀；其他流派条目、小运和合盘仍需单独规则文档与样本，不以假数据补位；
...
- P1：继续扩充经过规则验证的神煞、小运、完整 PWA / 离线排盘、日历视图增强；
