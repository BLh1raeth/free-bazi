...
- 神煞采用版本化的《三命通会》常用神煞表 v4，只展示可复核的规则命中；
- iOS 26/27 独立构建使用系统 Liquid Glass；Expo Go、Web 和不支持的系统自动使用轻量模糊降级；
- 无登录、无会员、无付费墙。

## 神煞准确性边界

当前开放 v4 固定目录中的 42 项规则；同柱多项完整保留，命中较多时可折叠或展开。固定口径、古籍文本来源与测试约束见 [`docs/shen-sha-standard-v4.md`](../docs/shen-sha-standard-v4.md)。界面只展示规则命中，不用神煞生成确定性吉凶。
