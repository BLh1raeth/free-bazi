...
function chartText(chart: BaziChart, anonymous: boolean): string {
  const pillars = chart.pillars
    .map((pillar) => pillar?.ganZhi ?? "时柱未知")
    .join("  ");
  const luck =
    chart.luckCycle?.items
      .map((item) => `${item.pillar.ganZhi} ${item.startYear}-${item.endYear}`)
      .join("；") ?? "未生成";
...
export function ExportActions({
  ...
  const filename = `元序-${new Date().toISOString().slice(0, 10)}`;
    ...
    try {
      ...
      if (kind === "png") download(dataUrl, `${filename}.png`);
        ...
        const pdf = new jsPDF({
          ...
          unit: "mm",
          format: [width, Math.max(297, height)],
        });
  ...
  return (
    <>
      ...
      <div className="export-actions" style={{ marginTop: 16 }}>
        <PrimaryButton
          type="button"
          onClick={() => copy(chartText(chart, anonymous))}
        >
          {busy === "已复制" ? "已复制" : "复制完整结果"}
        </PrimaryButton>
        <SecondaryButton
          ...
          onClick={() =>
            copy(
              chart.pillars.map((pillar) => pillar?.ganZhi ?? "未知").join(" "),
            )
          }
        >
          复制四柱
      ...
      <p className="small muted">
        图片与 PDF 库仅在点击后动态加载；导出不含二维码、广告或付费水印。
      </p>
    </>
  );
}
