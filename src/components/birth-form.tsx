    ...
    <GlassCard>
      <form onSubmit={submit} noValidate>
        <div className="form-grid">
          <div className="field">
            <label htmlFor="name">姓名或称呼（可选）</label>
            <input
              id="name"
              className="input"
              value={input.name}
          ...
          <div className="field">
            ...
            {input.gender === "unspecified" && (
              <span className="field-help">未指定时不生成大运。</span>
            )}
          </div>

          <div className="field field-full">
            <span className="field-label">历法类型</span>
            <SegmentedControl
          ...
          <details className="field field-full">
            <summary className="field-label">高级选项与显示设置</summary>
            <div className="check-row" style={{ marginTop: 14 }}>
              <label className="check">
                <input
                  type="checkbox"
                  checked={input.dayBoundaryRule === "lateZiNextDay"}
                  onChange={(e) =>
                    update(
                      "dayBoundaryRule",
                      e.target.checked ? "lateZiNextDay" : "midnight",
                    )
                  }
                />
                23:00 起换日
              ...
              <label className="check">
                <input
                  type="checkbox"
                  checked={input.showHiddenStems}
                  onChange={(e) => update("showHiddenStems", e.target.checked)}
                />
                显示藏干
              </label>
              <label className="check">
                <input
                  type="checkbox"
                  checked={input.showTenGods}
                  onChange={(e) => update("showTenGods", e.target.checked)}
                />
                显示十神
              </label>
              <label className="check">
                <input
                  type="checkbox"
                  checked={input.showNaYin}
                  onChange={(e) => update("showNaYin", e.target.checked)}
                />
                显示纳音
              </label>
              <label className="check">
