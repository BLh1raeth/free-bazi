...
test("用户可完成排盘并无刷新切换时间层级", async ({ page }) => {
  await page.goto("/");
  await expect(page.getByRole("heading", { name: /传统八字排盘工具/ })).toBeVisible();
  await page.getByLabel("出生年份").fill("1990");
  await page.getByLabel("出生月份").fill("6");
  await page.getByLabel("出生日期").fill("15");
  await page.getByRole("button", { name: "生成免费命盘" }).click();
  ...
  await expect(page.locator("#export-document").getByRole("heading", { name: "四柱命盘" })).toBeVisible();
  await expect(page.locator("#export-document").getByRole("heading", { name: "大运" })).toBeVisible();
});

test("移动端表单与免费承诺可读", async ({ page }, testInfo) => {
  test.skip(testInfo.project.name !== "mobile", "只在移动端项目验证");
  await page.goto("/");
  await expect(page.getByText("本站所有排盘功能永久免费", { exact: false }).first()).toBeVisible();
  await expect(page.getByRole("button", { name: "生成免费命盘" })).toBeVisible();
});
